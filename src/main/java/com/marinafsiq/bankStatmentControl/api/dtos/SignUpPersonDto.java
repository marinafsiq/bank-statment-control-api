...
public class SignUpPersonDto {
	...
	private Optional<List<Card>> cards = Optional.empty(); //optional. não precisa adicionar Cards ao ao fazer sign in. Isso pode ser adicionado posteriormente.
	...
	public void setCards(Optional<List<Card>> cards) {
		this.cards = cards;
	}
	...
	public String toString() {
		String res = String.format("PersonDto: {id=%d, name=%s, email=%s, password=%s, cpf=%d, num of cards=%d, num of accounts=%d}",
						id, name, email, password, cpf, cards.get().size(), accounts.get().size());
		return res;
	}
